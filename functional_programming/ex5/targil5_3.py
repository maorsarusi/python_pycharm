#
# Targil 5  
# Targil 4 _1 by generatoe instead of list comperhension
...
def listsOfStu1(L, L1):
    return list([L1[i][0], [L[j][0]]] for i in range(0, len(L1)) for j in range(0, len(L)) if L1[i][1] == L[j][2])


# method for get teachers witout any student
def listsOfStu2(L, L1):
    return list([L1[i][0], []] for i in range(0, len(L1)) if not L1[i][1] in L)
...
def StudendsGradesByTeachers(L, L1):
    return list([L1[i][0], [L[j][1]]] for i in range(0, len(L1)) for j in range(0, len(L)) if L1[i][1] == L[j][2])
...
#
def myStuList(L, L1):
    return (stdevByTeacher(
        avgByTeacher(unionByTeacher(listsOfStu1(L, L1)), unionByTeacher(StudendsGradesByTeachers(L, L1))),
        unionByTeacher(StudendsGradesByTeachers(L, L1))) +

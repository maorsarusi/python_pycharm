...
def printingAll(fldict):
  return  list([print("sentence",list(fldict[i][1].keys()),
                 "LineNr:",fldict[i][0],"nr of vowels:",createingOneTuple(list(zip(*[item for item in list(fldict[i][1].values())]))[0]),
                 "nr of b-m consonants:", createingOneTuple(list(zip(*[item for item in list(fldict[i][1].values())]))[1]),
                 "nr of n-z consonants:",createingOneTuple(list(zip(*[item for item in list(fldict[i][1].values())]))[2])) for i in range(0,len(fldict))])
